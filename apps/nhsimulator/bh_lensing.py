...
def simulate_lensing(default_params,source_mag_range,tE_range,log):
    ...
    if 'horizons_file' in default_params.keys():
        ...
        spacecraft_positions = jplhorizons_utils.calc_norm_spacecraft_positions(horizons_table,default_params['to'],normalize=False)
    ...
    for j in range(0,len(source_mag_range),1):
        ...
        for i in range(0,len(tE_range),1):
            ...
            (lc_no_parallax,sim_e_no_parallax) = NH_data_simulator.add_event_to_lightcurve(baseline_lc,
                                                                       ...
                                                                       output_lc=True,annual_parallax=True)
            ...
            fit_no_parallax = NH_lensing.FitParams()
            fit_parallax = NH_lensing.FitParams()
            e_no_parallax = None
            e_parallax = None
            ...
            plot_lightcurves(lc_no_parallax,lc_parallax,file_path)

def plot_lightcurves(lc_no_parallax,lc_parallax,file_path):
    ...
    plt.plot(ts_no_parallax,lc_no_parallax[:,1],color='#8c6931',linestyle='solid',
                label='Sun')
                
    plt.errorbar(ts_no_parallax,lc_no_parallax[:,1],
                 yerr=lc_no_parallax[:,2],alpha=1.0,color='#8c6931',fmt='none')
    
    plt.plot(ts_parallax,lc_parallax[:,1],color='#2b8c85',linestyle='solid',
                label='New Horizons')
                
    plt.errorbar(ts_parallax,lc_parallax[:,1],
                 yerr=lc_parallax[:,2],alpha=1.0,color='#2b8c85',fmt='none')
