...
def plot_sn_matrix(sim_data,log_dir,default_params):
    ...
    plt.subplots_adjust(top=0.90,left=0.02,right=0.98,bottom=0.4)
    ...
    cbar = plt.colorbar(orientation='horizontal',fraction=0.125,pad=0.45)
    
    cbar_step = max_res.max()/5.0
    
    xticks = []
    for x in np.arange(0.0, (max_res.max()+cbar_step), cbar_step):
        xticks.append(str(round(x,7)))
    cbar.ax.set_xticklabels(xticks, rotation=45.0)
