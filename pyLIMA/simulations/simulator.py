...
def simulate_microlensing_model_parameters(model):
    ...
    fake_fluxes_parameters = simulate_fluxes_parameters(model.event.telescopes, source_magnitude=[10, 20],
...
def simulate_lightcurve_flux(model, pyLIMA_parameters, add_noise=True, exposure_times=None):
    ...
    for ind, telescope in enumerate(model.event.telescopes):
        ...
        if telescope.lightcurve_flux is not None:
            ...
            if add_noise:
                ...
                observed_flux, err_observed_flux = brightness_transformation.noisy_observations(theoritical_flux, exposure_times)
