...
def test_annual_parallax():
    ...
    np.allclose(np.array([[-11.99055572, -11.40251147], [26.46632345, 25.16787805]]),
                positions)
...
def test_terrestrial_parallax():
    ...
    np.allclose(np.array(
        [[-4.68497968e-21, 2.65705901e-21], [-3.81036079e-05, -4.12319764e-05]]),
                positions)


def test_space_parallax():
    event = _create_event()
    event.telescopes[0].lightcurve_flux[:, 0] = 2458000.5 + event.telescopes[
                                                                0].lightcurve_flux[:, 0]
    parallax_model = ['None', 1664.51]

    parallax = microlparallax.MLParallaxes(event, parallax_model)

    positions = parallax.space_parallax(event.telescopes[0].lightcurve_flux[:, 0],
                                        'Kepler', event.telescopes[0])

    np.allclose(np.array([[-0.26914499, -0.40329631], [0.63675129, 0.93466804]]),
                positions)
...
def test_parallax_combination_on_Earth_annual():
    ...
    positions_annual = parallax.annual_parallax(
        event.telescopes[0].lightcurve_flux[:, 0])
...
def test_parallax_combination_on_Earth_full():
    ...
    positions_annual = parallax.annual_parallax(
        event.telescopes[0].lightcurve_flux[:, 0])
    positions_terrestrial = parallax.terrestrial_parallax(
        event.telescopes[0].lightcurve_flux[:, 0],
        0, 0, 0)
    parallax.parallax_combination(event.telescopes[0])

    np.allclose(positions_annual + positions_terrestrial,
...
def test_parallax_combination_on_Space():
    ...
    positions_annual = parallax.annual_parallax(
        event.telescopes[0].lightcurve_flux[:, 0])
    positions_space = parallax.space_parallax(event.telescopes[0].lightcurve_flux[:, 0],
                                              'Kepler', event.telescopes[0])

    parallax.parallax_combination(event.telescopes[0])

    np.allclose(positions_annual + positions_space,
                event.telescopes[0].deltas_positions)
...
def test_compute_parallax_curvature():
    ...
    parallax.parallax_combination(event.telescopes[0])
    piE = [0.5, 0.5]
    curvature = microlparallax.compute_parallax_curvature(piE, event.telescopes[
        0].deltas_positions)

    dtau = piE[0] * event.telescopes[0].deltas_positions[0] + piE[1] * \
           event.telescopes[0].deltas_positions[1]
    du = piE[1] * event.telescopes[0].deltas_positions[0] - piE[0] * \
         event.telescopes[0].deltas_positions[1]
