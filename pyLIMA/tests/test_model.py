...
#from pyLIMA.models import DFSPLmodel, DSPLmodel, FSBLmodel, FSPLmodel, FSPLargemodel, PSBLmodel, PSPLmodel, USBLmodel
...
def _create_event(JD=0, astrometry=False):
    ...
    event.telescopes[0].lightcurve_flux = time_series.construct_time_series(np.array([[JD+0,10,2],[JD+20,200,3]]), ['time','flux','err_flux'],
                                                                                     ['JD','W/m^2','W/m^2'])

    if astrometry:
        event.telescopes[0].astrometry = time_series.construct_time_series(
            np.array([[JD + 0, 10, 2, 10, 2], [JD + 20, 200, 3,200,3]]), ['time', 'ra', 'err_ra','dec','err_dec'],
            ['JD', 'deg', 'deg','deg','deg'])
        dico = {'astrometry': np.array([np.array([0.2, 0.1]), np.array([0.6, 0.98])])}
        event.telescopes[0].Earth_positions_projected.__getitem__.side_effect = dico.__getitem__
        event.telescopes[0].Earth_positions_projected.__iter__.side_effect = dico.__iter__
...
def test_DFSPL():
    ...
    assert dict(Model.model_dictionnary) == {'t0': 0, 'delta_t0': 2, 'u0': 1, 'delta_u0': 3, 'tE': 4, 'rho_1': 5,
                                             'rho_2': 6, 'q_flux_I': 7, 'fsource_Test': 8, 'fblend_Test': 9}
...
def test_DSPL():
    ...
    assert dict(Model.model_dictionnary) == {'t0': 0, 'u0': 1, 'delta_t0': 2, 'delta_u0': 3, 'tE': 4,
                                             'q_flux_I': 5, 'fsource_Test': 6, 'fblend_Test': 7}
    ...
    source1x,source1y,source2x,source2y = Model.sources_trajectory(event.telescopes[0], pym)
...
def test_PSPL():
    ...
    assert np.allclose(jacobi, np.array([[-1.46870667e+02, -8.48324973e+02,  1.93250878e+00],
