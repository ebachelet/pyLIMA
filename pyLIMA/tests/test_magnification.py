...
def test_magnification_PSPL_Jacobian():
    ...
    jacobian, magnification = magnification_Jacobian.magnification_PSPL_Jacobian(pspl, telo, pym)
...
def test_magnification_FSPL_Jacobian():
    ...
    assert np.allclose(jacobian,[-1.71720554e-01, -1.02195194e+02,  9.95481471e-04, 7.42711447e+00])
...
def test_magnification_numerical_Jacobian():
    ...
    assert np.allclose(jacobian, [-1.87252646e-01, -1.02197457e+02,  9.95504683e-04,  7.43527365e+00]) #not as precise...
...
def test_magnification_PSPL():
    ...
    magnification, impact_parameter = magnification_PSPL.magnification_PSPL(tau, uo, return_impact_parameter=True)
...
def test_magnification_FSPLarge():
    ...
    magnification_fspl = magnification_VBB.magnification_FSPL(tau, uo, rho, limb_darkening_coefficient,
...
def test_magnification_USBL():
    ...
    magnification = magnification_VBB.magnification_USBL(separation, mass_ratio, x_source, y_source, rho)
...
def test_magnification_FSBL():
    ...
    magnification = magnification_VBB.magnification_FSBL(separation, mass_ratio, x_source, y_source, rho,
...
def test_magnification_PSBL():
        ...
        magnification = magnification_VBB.magnification_PSBL(separation, mass_ratio, x_source, y_source)
