...
def test_orbital_motion2D():
    ...
    dalpha = orbital_motion.orbital_motion_2D.orbital_motion_2D_trajectory_shift(time, t0_om, dalpha_dt)
    ...
    dseparation = orbital_motion.orbital_motion_2D.orbital_motion_2D_separation_shift(time, t0_om, dalpha_dt)
...
def test_orbital_motion_circular():
    ...
    pyLIMA_parameters = collections.namedtuple('parameters', ['Rmatrix','orbital_velocity','a_true','t_periastron',
                                                              'separation','alpha'])
    ...
    dsep,dalpha = orbital_motion.orbital_motion_3D.orbital_motion_keplerian(time, pyLIMA_parameters, ['Circular',2458555])
...
def test_orbital_motion_circular():
    ...
    pyLIMA_parameters = collections.namedtuple('parameters',
                                               ['Rmatrix', 'orbital_velocity', 'a_true', 't_periastron','eccentricity',
                                                'separation', 'alpha'])
    ...
    dsep, dalpha = orbital_motion.orbital_motion_3D.orbital_motion_keplerian(time, pyLIMA_parameters,
                                                                             ['Keplerian', 2458555])
...
def test_orbital_parameters_from_position_and_velocities():
    ...
    outputs = orbital_motion.orbital_motion_3D.orbital_parameters_from_position_and_velocities(separation_0, r_s, a_s, v_para, v_perp, v_radial, t0_om)
    assert np.allclose(outputs[:-3],(0.2622029190714234, 1.8293736686171367, -1.5463618954109095, 2.1002142747824566,
...
def test_state_orbital_elements():
    ...
    outputs = orbital_motion.orbital_motion_3D.state_orbital_elements(separation_0, r_s, a_s, v_para, v_perp, v_radial)
    ...
    assert np.allclose(outputs[4:], (1.7501785623187138, 1.225, 2.1002142747824566, 5.329996919004271, 0.7585218053347751))

def test_eccentric_anomaly_function():

    ecc = orbital_motion.orbital_motion_3D.eccentric_anomaly_function([2458955], 0.27, 2456589, 3.2)
