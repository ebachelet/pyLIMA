...
def _create_event():
    ...
    event.telescopes[0].lightcurve_magnitude = np.array(
        [[0, 1, 1], [42, 6, 6], [43, 5, 1], [54, 8, 6]])
    event.telescopes[0].lightcurve_flux = np.array(
        [[0, 1, 1], [42, 6, 6], [43, 5, 1], [54, 8, 6]])
...
def test_differential_evolution_parameters_boundaries_PSPL():
    ...
    parameters_boundaries = microlguess.differential_evolution_parameters_boundaries(
        model)
...
def test_MCMC_parameters_initialization():
    ...
    parameters_dictionnary = {'to': 0, 'uo': 1, 'tE': 2, 'rho': 3, 'fs_LCOGT': 4,
                              'g_LCOGT': 5}

    parameter_key_0 = 'to'
    trial_0 = \
    microlguess.MCMC_parameters_initialization(parameter_key_0, parameters_dictionnary,
                                               parameters)[0]
    assert (trial_0 > -1.0) & (trial_0 < 1.0)

    parameter_key_1 = 'uo'
    trial_1 = \
    microlguess.MCMC_parameters_initialization(parameter_key_1, parameters_dictionnary,
                                               parameters)[0]
    assert (trial_1 > 0.9 * 1.1) & (trial_1 < 1.1 * 1.1)

    parameter_key_2 = 'tE'
    trial_2 = \
    microlguess.MCMC_parameters_initialization(parameter_key_2, parameters_dictionnary,
                                               parameters)[0]
    assert (trial_2 > 0.9 * 2.2) & (trial_2 < 1.1 * 2.2)

    parameter_key_3 = 'rho'
    trial_3 = \
    microlguess.MCMC_parameters_initialization(parameter_key_3, parameters_dictionnary,
                                               parameters)[0]
    assert (trial_3 > 0.1 * 3.3) & (trial_3 < 10 * 3.3)

    parameter_key_4 = 'fs_LCOGT'
    trial_4 = microlguess.MCMC_parameters_initialization(parameter_key_4,
    ...
    trial_5 = microlguess.MCMC_parameters_initialization(parameter_key_5,
                                                         parameters_dictionnary,
                                                         parameters)
    assert len(trial_4) == 1
