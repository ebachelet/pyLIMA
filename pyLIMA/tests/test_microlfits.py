...
def _create_event():
    ...
    event.total_number_of_data_points.return_value = sum(
        [len(i.lightcurve_flux) for i in event.telescopes])
...
def _create_model(kind):
    ...
    model.compute_the_microlensing_model.return_value = np.random.random(
        100).tolist(), 0.0, 0.0
    ...
    model.pyLIMA_standard_parameters_to_fancy_parameters.return_value = \
        fancy_namedtuple(
        10.0, 0.1, 20, 10, 5)
    ...
    model.compute_pyLIMA_parameters.return_value = fancy_namedtuple(10.0, 0.1, 20, 10,
                                                                    5)
...
def test_mlfit_PSPL_LM_without_guess():
    ...
    assert fit.fit_covariance.shape == (
    3 + 2 * len(current_event.telescopes), 3 + 2 * len(current_event.telescopes))
...
def test_mlfit_FSPL_LM_without_guess():
    ...
    assert fit.fit_covariance.shape == (
    4 + 2 * len(current_event.telescopes), 4 + 2 * len(current_event.telescopes))
...
def test_mlfit_DSPL_LM_without_guess():
    ...
    assert fit.fit_covariance.shape == (
    6 + 2 * len(current_event.telescopes), 6 + 2 * len(current_event.telescopes))
