...
def test_find_2_lenses_caustics_and_critical_curves():
    ...
    regime, caustics, crit_curve = binary_caustics.find_2_lenses_caustics_and_critical_curves(separation, mass_ratio,
                                                                                             resolution=5)
    assert regime == 'close'

    assert np.allclose(caustics[0], np.array([ 0.01628524+1.30963236e-32j,  0.00247-7.14705680e-03j,
    ...
    assert np.allclose(caustics[1], np.array([-2.95002726+2.39678545j, -2.94583595+2.39903663j,
       -2.94527278+2.39804629j, -2.9445595 +2.39425766j,
       -2.95002726+2.39678545j]))

    assert np.allclose(caustics[2], np.array([-2.95002726-2.39678545j, -2.9445595-2.39425766j,
    ...
    assert np.allclose(crit_curve[0], np.array([ 1.01181895-3.05338437e-17j,  0.71307147+6.98075159e-01j,
    ...
    assert np.allclose(crit_curve[1], np.array([0.19403416-0.07778688j, 0.19590983-0.07507009j,
       0.19872016-0.07665976j, 0.19705122-0.07977972j,
       0.19403416-0.07778688j]))

    assert np.allclose(crit_curve[2], np.array([0.19403416+0.07778688j, 0.19705122+0.07977972j,
...
def test_sort_2lenses_resonant_caustic():
    ...
    resonant_caustic, resonant_cc = binary_caustics.sort_2lenses_resonant_caustic(caustic_points, critical_curves_points)

    assert np.allclose(resonant_caustic, np.array([0. +1.j , 0.4+0.4j, 0.4-0.4j, 0. -1.j ]))
    assert np.allclose(resonant_cc, np.array([0. +0.5j, 4.5+0.j , 4.5-0.j , 0. -0.5j]))
...
def test_sort_2lenses_close_caustics():
    ...
    central_caustic, close_top_caustic, close_bottom_caustic, central_cc, close_top_cc, close_bottom_cc = \
        binary_caustics.sort_2lenses_close_caustics(caustic_points, critical_curves_points)
...
def test_sort_2lenses_wide_caustics():
    ...
    central_caustic, wide_caustic, central_cc, wide_cc = binary_caustics.sort_2lenses_wide_caustics(caustic_points,
...
def test_compute_2_lenses_caustics_points():
    ...
    caustics, critical_curves = binary_caustics.compute_2_lenses_caustics_points(separation, mass_ratio, resolution=2)

    assert np.allclose(caustics, np.array([[-2.57080794e-04+1.80313310e-31j,  4.66488170e-01+3.88809819e-28j,
    ...
    assert np.allclose(critical_curves, np.array([[-1.0002174 -3.89464757e-17j,  1.26590509+1.76169205e-15j,
...
def test_poly_binary_eiphi_0():
    ...
    assert np.allclose(polynomial, [1.0, -1.56, -0.8316000000000001, 1.9032000000000002, -0.7799999999999999])
...
def test_caustic_points_at_phi_0():
    ...
    assert np.allclose(caustic_points,np.array([-0.17212486+0.j        ,  0.66178081+0.j        ,
        0.01111935-0.23897312j,  0.01111935+0.23897312j]))
...
def test_lens_equation():
    ...
    assert np.allclose(zetas, np.array([ -0.13544576+0.70262628j, -57.98235531+1.99937622j]))