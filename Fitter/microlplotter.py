...
class MLPlotter(object) :
    ...
    def __init__(self, event):
        ...
        self.event = event
        self.filters_dictionnary = {'N': 'NIR', 'I':'NIR', 'Red':'NIR', 'V':'V', 'J':'IR', 'H':'IR', 'K':'IR'}
    ...
    def find_observed_bands(self):
        ...
        self.observed_bands = np.unique([self.filters_dictionnary[i.filter] for i in self.event.telescopes])
    ...
    def initialize_plots(self, choice, observe):
        ...
        for i in self.observed_bands:
            ...
            axes = plt.gca()
            limitmoins = self.event.fits_results[choice][3][plot_model.model_dictionnary['to']]-1.5*self.event.fits_results[choice][3][plot_model.model_dictionnary['tE']]
            limitplus = self.event.fits_results[choice][3][plot_model.model_dictionnary['to']]+1.5*self.event.fits_results[choice][3][plot_model.model_dictionnary['tE']]
    ...
    def plot_lightcurves_mag(self, align):
        ...
        for i in self.event.telescopes:
            ...
            if align == 'Yes' :
                ...
                err_mag = -2.5*i.lightcurve_flux_aligned[:,2]/(i.lightcurve_flux_aligned[:,1]*np.log(10))
    ...
    def plot_model_mag( self, choice):
        ...
        for i in self.observed_bands :
            ...
            ampli=microlmagnification.amplification( plot_model, t, parameters, self.event.telescopes[0].gamma)[0]
            ...
            plt.plot(t, 27.4-2.5*np.log10(fs*ampli+fb),self.color_dictionnary[self.event.fits_models[choice][2].paczynski_model], lw=2,label= self.event.fits_models[choice][2].paczynski_model)
            plt.legend(numpoints=1)
    ...
    def plot_model_mag_uncertainties (self, choice) :
        ...
        for i in self.observed_bands :
            ...
            for j in uncertain :
                ...
                parameters = self.event.fits_results[choice][3][:-1]+j*self.event.fits_covariance[choice][3].diagonal()**0.5
                if self.event.telescopes[0].lightcurve[0, 0] > 2450000:
    ...
    def align_lightcurves(self, choice) :
        ...
        for i in self.event.telescopes :
            ...
            flux_align = (i.lightcurve_flux[:, 1]-self.event.fits_results[choice][3][plot_model.model_dictionnary[
            'fs_'+i.name]]*self.event.fits_results[choice][3][plot_model.model_dictionnary[
            'g_'+i.name]])/self.event.fits_results[choice][3][plot_model.model_dictionnary[
            'fs_'+i.name]]*self.event.fits_results[choice][3][plot_model.model_dictionnary[
            'fs_'+self.telescopes_reference[self.filters_dictionnary[i.filter]]]]+self.event.fits_results[choice][3][plot_model.model_dictionnary[
            'fs_'+self.telescopes_reference[self.filters_dictionnary[i.filter]]]]*self.event.fits_results[choice][3][plot_model.model_dictionnary[
